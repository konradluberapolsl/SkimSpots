        ...
        <ScrollView  contentContainerStyle={styles.scrollContainer} onLayout={onLayout}>
            ...
            {commentsReady ?(
                    comments!!.slice(0, numberOfCommentsToShow).map((comment, index) => {
                            return (
                                <CommentItem style={styles.commentsContainer} key={index}  comment={comment} />
                            )})
                ):
                ( comments.length != 0 && <ActivityIndicator size="large" color={foregroundColor} />) }

            <OwnComment text={text} onChaneText={onTextChange} userName={user!!.name} style={styles.commentsContainer} />
            { text!="" && <Button style={styles.button} onPress={onPostCommentButtonClicked} uppercase={false}>Zostaw komentarz</Button>  }
            { comments.length>numberOfCommentsToShow && text=="" &&
            <Button
                style={styles.button}
                uppercase={false}
                onPress={() => props.navigation.navigate("AllPlaceCommentsScreen", {
                    place: place!!,
                    comments: comments!!
                })}
            >Zobacz wszystkie komentarze!</Button>
            }
        </ScrollView>
        </SafeAreaView>
    );
